...
class Linear(nn.Linear):
    def forward(self, input):
        ...
        out = torch.nn.functional.linear(
            torch.nn.functional.dropout(input, 0.5, training=self.training),
            self.weight,
            bias,
        )
        return out
    ...
    def fit(
        self, Xtr, ytr, criterion, reg=0.0, epochs=100, optimizer=None, use_cuda=False
    ):
...
def train_and_eval_linear(
    X_tr, y_tr, X_val, y_val, X_te, y_te, num_class, stratified_indices, use_cuda=False
):
    embed_dim = X_tr.shape[1]
    search_grid = 2.0 ** np.arange(3, 18)
